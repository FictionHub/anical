...
console.log("     AND into VAPID_PUBLIC_DEFAULT at the top of netlify/functions/push-send.mjs —");
console.log("     the browser signs up with one and the server signs with the other, so a pair");
console.log("     that only half-lands means every notification is silently rejected.");
...
console.log("  3. Verify without sending anything:");
console.log("     curl \"https://<site>/.netlify/functions/push-send?secret=$CRON_SECRET&dry=1\"");
