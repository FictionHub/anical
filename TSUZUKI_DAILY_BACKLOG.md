...
**Second pass, later in August.** Two days had shipped without the file saying so — Day 53 was live, and Day 88 had shipped out of its own calendar order, alongside the rest of the page it lives on — and the "Picking This Up Cold" invariants below had gone stale under a site restructure that happened the same week. Fixed below; nothing renumbered.
...
One page has already had exactly that done to it. `/today/` is the one SEO page whose entire premise expires every 24 hours, and a weekly regenerate isn't fast enough to keep it honest — it named the wrong Thursday for four days straight before `today.mjs` replaced it with a request-time render off the same catalog cache the rest of the API shares. The rest of the SEO pages, and the `.ics` feeds, are still deploy-and-weekly-regenerate bound; `today.mjs` is the pattern, not yet the policy.
...
That division held for exactly one month. Fourteen roadmap slots were overtaken by work that shipped early — the public API, the theming engine, release variants, web push, a grounded assistant, a client-side taste-and-recommendation engine — and were rescoped in place rather than deleted. The knock-on for *this* document is the `◐` days: where the heavy version arrived first, the light version below is no longer the thing to build, so the entry now describes the user-facing half that the backend shipped without.
...
**And a lens added later that the other four can't stand without: 🎞 Feed.** Every ranked lens on this page needs a taste profile to rank against, and a brand-new visitor has none — so `Feed` sits outside that gate, one show at a time, and is what actually builds the library the rest of the page ranks. It's the default lens for exactly that reason: the front door has to be the one that works from nothing. `Loved it` / `Liked it` / `Not for me` writes a real score, the anchor the whole profile hangs off; `Watching` / `Plan to watch` / `Dropped` writes a status instead, which reaches the taste vectors as weak signal at a third of a rating's weight — enough to inform the profile without letting a swiping session outvote the scores you actually sat down and gave. Nothing in the original 365 asked for this; the cold-start problem only became visible once Days 32–54 existed and there was a gate to get stuck behind. Filed here because it lives on this page, not because a day predicted it — the same honesty Day 31 already applied to `anical.activity`.
...
### Day 53 · Sep 22 | Retrain in place ✅ shipped
...
**A second input, not an edit of the output — the same split as Day 13's axes/composite and Day 17's stored/displayed score.** `anical.tasteadj` holds `dim|value -> delta`, layered onto the lift at read time rather than written back into the taste vectors themselves. Writing the corrected lift straight into the vectors would have been one line, and the next rating you gave would have silently erased it. Keeping it a separate, additive delta is also the only reason a reset is possible at all: the derived value underneath was never touched, let alone lost.

**A step, not a number.** Each nudge moves the delta by 0.25, clamped at ±2 — past that you are no longer correcting the model's read of you, you are overriding it outright, and the cap keeps the two visually distinct. A nudge back to exactly zero deletes the key rather than storing a `0`, the same rule Day 14's sliders use for an unscored axis: an explicit "no delta" and "no opinion at all" have to stay distinguishable on the next read.

**One write invalidates both caches it can invalidate.** Saving a taste adjustment clears the taste-vector cache and the prediction cache together, because both are derived from the same lift this changes — clearing one and leaving the other stale would mean the chart under your thumb updates and the card behind it doesn't.
...
### Day 88 · Oct 27 | Seasonal preview ✅ shipped
...
**Shipped out of order, alongside Day 46's page rather than waiting for its own calendar slot.** `?view=recs` refuses to rank its other lenses without a taste profile to rank against — but a season preview is a browsing list first and a prediction second, so `Next season` is deliberately let through that gate with no profile at all. A show nothing can predict from just renders unpredicted; the page doesn't refuse to open on a first-time visitor who hasn't rated eight things yet.
