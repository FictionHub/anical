...
- **The skin catalogue is generated, and its ids are permanent.** `scripts/build-themes.mjs` holds one recipe row per AniList id — a motif, a display face, a shape preset, a rarity, sometimes a colour override — and writes `site/data/themes.json`, which the live Blobs layer is merged on top of at read time. **75 skins as of Aug 2026** (1 exclusive, 7 legendary, 15 epic, 22 rare, 30 common); adding more is a row and a re-run, and `--check` verifies every remote URL still resolves before it ships. Two things bite. A theme's id is slugified from its title and is what a wallet stores when someone owns that skin, so **renaming a title later orphans every ownership row pointing at it**. And an `exclusive` never moves into a buyable tier — that is the single edit that retroactively takes something away from a person who already has it. The served catalogue is 214KB raw and 13KB brotli, fetched once when the Skins panel opens.
...
> `ui` — **Done when** the five sliders accept half-points and the composite updates as you drag. **Needs** Day 13. **Read Day 13 first**: the whole write path (`setAxis`, `cleanAxis`, `compositeOf`) and the labels (`AXIS_DEFS`) exist — this day is the sliders and the live read-out, not the model.

### Day 15 · Aug 15 | Composite weights
Drag how much each axis counts toward your score, stored per user. `[W04]`
> `ui` — **Done when** changing the weights recomputes every composite in the library at once. **Needs** Day 14. **The hook is already there**: `compositeOf()` asks `axisWeights()` rather than reading a constant, so the recompute is that one function plus a sweep over `state.axes` — nothing else reads the weights directly.
